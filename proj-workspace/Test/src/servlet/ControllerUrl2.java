 ...
 */
@WebServlet("*.dots")
public class ControllerUrl2 extends HttpServlet
{
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
	{
		try {
			...
			// + /view1.do
			// url Í≤ΩÎ°ú ?ôï?ù∏?ïòÍ∏? ?úÑ?ïú Î¨∏Ïûê?ó¥ ?û¨Í∞?Í≥?
			String path = request.getRequestURI().replaceAll(request.getContextPath(), "");
			String url = null;		
			System.out.println(path);
			
			Action action = null;
			if (path.equals("/*.do") || path.equals("/view1.dots")) {
				action = new View1Action();
				action.execute(request, response);
				
				url = "/WEB-INF/views/view1.jsp";
				
			} else if (path.equals("/view5.dots")) {
